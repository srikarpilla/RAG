...
def get_resp(query, context):
    ...
    # Create a comprehensive prompt with context and instructions
    ...
    try:
        ...
        for model in available_models:
            try:
                ...
                st.sidebar.warning(f"Model {model} failed, trying next...")
                continue
        ...
        with st.spinner("Searching for relevant context and generating answer..."):
            ...
            with st.expander("📚 View Relevant Context Used"):
                for i, c in enumerate(context, 1):
                    ...
                    # Display first 300 characters with option to see more
                    if len(c) > 300:
                        st.text(c[:300] + "...")
...
st.sidebar.markdown("---")
st.sidebar.info("Using Cohere's Command models for generation")